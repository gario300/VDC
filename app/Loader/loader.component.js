...
export default Loader = props => {
    ...
    React.useEffect( () => {
        ...
        const OnLoaderUpdated = () => {
            const loaderState = myAppState.getLoaderState;
            setLoaderState({
        ...
        AppStore.on("loaderUpdated", OnLoaderUpdated);

        return () => {
            AppStore.removeListener("loaderUpdated", OnLoaderUpdated);
        }
    }, []);

    return (
        <View style={[styles.loaderContainer, loaderState.isActive && styles.loaderActive]}>
            {loaderState.type === 1 && <View style={styles.loaderNormal}>
